...
def get_market_data(market_info, target_date):
    ...
    try:
        ...
        if df.empty:
            print(f"[{name}] 🚨 逻辑判断: yfinance 返回了空数据 (DataFrame empty)。可能该代码无效或API无响应。")
            return None
        ...
        if target_date not in df.index:
            print(f"[{name}] 💤 判断依据: 目标日期 {target_date} 不在 yfinance 返回的有效交易日索引中。")
            print(f"[{name}] 💤 逻辑判断: 判定为该市场在当日休市，跳过计算。")
        ...
        try:
            ...
            if idx > 0:
                ...
                print(f"[{name}] 成功找到前一交易日 ({all_dates[idx-1]}) 数据作为基准价。")
            else:
                prev_close = target_row['Open']
                print(f"[{name}] ⚠️ 未找到前一交易日数据，使用当日开盘价作为基准价。")
        except ValueError:
